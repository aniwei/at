...
import { clamp, lerp, sign } from '../basic/helper'
import { Offset } from '../basic/geometry'
import { At } from '../at'

export abstract class AtParametricCurve<T> {
  ...
  toString () {
    return `AtParametricCurve()`
  }
}


export abstract class AtCurve extends AtParametricCurve<number> {
  ...
  public get flipped (): AtCurve {
    return AtFlippedCurve.create(this)
  }
}


export class AtLinear extends AtCurve {
  static create () {
    return new AtLinear()
  }
...
export class AtSawTooth extends AtCurve {
  public count: number
...
export class AtInterval extends AtCurve {
  public begin: number
  public end: number
  public curve: AtCurve 

  constructor (begin: number, end: number, curve: AtCurve) {
    super()
    this.begin = begin
    this.end = end
    this.curve = AtCurves.linear
  }
...
export class AtThreshold extends AtCurve {
...
export class AtCubic extends AtCurve {
...
class AtThreePointCubic extends AtCurve {
  static create (
    a1: Offset,
    b1: Offset,
    midpoint: Offset,
...
abstract class AtCurve2D extends AtParametricCurve<Offset> {
  generateSamples(
    start: number = 0.0,
    end: number = 1.0,
    ...
    const first = AtCurve2DSample.create(start, this.transform(start))
    const last = AtCurve2DSample.create(end, this.transform(end))
    const samples: AtCurve2DSample[] = [first]

    const sample = (p: AtCurve2DSample, q: AtCurve2DSample, forceSubdivide: boolean = false) => {  
      const t = p.t + (0.45 + 0.1 * Math.random()) * (q.t - p.t)
      const r = AtCurve2DSample.create(t, this.transform(t))
...
class AtCurve2DSample {
  static create (t: number, value: Offset) {
    return new AtCurve2DSample(t, value)
  }
...
class AtCatmullRomSpline extends AtCurve2D {
  static create (
...
export class AtCatmullRomCurve extends AtCurve {
  ...
  static computeSamples (controlPoints: Offset[], tension: number): AtCurve2DSample[] {
...
export class AtFlippedCurve extends AtCurve {
  static create (curve: AtCurve) {
...
export class AtDecelerateCurve extends AtCurve {
  static create () {
...
export class AtBounceInCurve extends AtCurve {
  static create () {
...
export class AtBounceOutCurve extends AtCurve {
  static create () {
    return new AtBounceInCurve()
  }
...
export class AtBounceInOutCurve extends AtCurve {
  static create () {
    return new AtBounceInCurve()
  }
...
export class AtElasticInCurve extends AtCurve {
...
export class AtElasticOutCurve extends AtCurve {
  static create (period?: number) {
...
export class AtElasticInOutCurve extends AtCurve {
  static create (period?: number) {
